...
# Simple build system
SBS is a light weight build system for C/C++ projects using gnu-make and either gnu compilers (gcc/g++) or LLVM compilers (clang/clang++). It is intended for users who want an easy to install and configure build system that doesn't require the user to write complex makefiles, has minimal dependencies and doesn't add any steps to the build process. SBS strives for simplicity and ease of use, making most of the build settings a matter of simple configuration, sparing the coder the need to write Makefile rules and for the most part, even understand how Makefiles work.
...
Many of the configuration settings are explained in detail. This might look intemidating for the less experieced users. Yet for most of the configuration tasks, just getting them to work should be easy enough, even without understanding exactly what happens under the hood, so don't fret.
...
### Executable
For an `exec` `MODULE_BIN_TYPE` value, SBS will build an executable. This is the default value if `MODULE_BIN_TYPE` is not set. `exec` binary type doesn't modify any build flags.

### Shared library
For a `shared` `MODULE_BIN_TYPE` value, SBS wile build a shared library, where:
...
### Static library
For a `static` `MODULE_BIN_TYPE` value, SBS will build a static library, where:
...
### None
For a `none` `MODULE_BIN_TYPE` value, SBS will not build any artifact. It will compile all the source files in the `MODULE_SRCS` list, but will not link them. This type doesn't modify any build flags.
