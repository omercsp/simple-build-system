...
# Simple Build System
SBS is a light weight build system for C/C++ projects using gnu-make and either gnu compilers (gcc/g++) or LLVM compilers (clang/clang++). It is intended for users who want an easy to install and configure build system without the need to write complex makefiles, doesn't require additional dependencies and doesn't add any steps to the build process. SBS strives for simplicity and ease of use, making most of the build settings a matter of simple configuration, sparing the coder the need to write Makefile rules and for the most part, even understand how Makefiles work.
...
```
And that's it. We're done with setting up the build, and ready to build the project, and run the created executable (we could have gotten away without the 1st line, but it's probably clearer this way).
...
There are quite a few settings, allowing the user to control many aspects of the build. Guidelines and the more common tasks and settings are covered below. **For a full list of possible settings and their documentation, see the [Makefile.skel](Makefile.skel) file**.

Each SBS Makefile must source (using `include`) the `module.inc.mk` file. Its location is up to the user to decide. A reasonable place to put it is in the project root directory.
...
Source files can reside anywhere, as long as they accessible. SBS treats differently source files inside or outside the module's directory. Both types of files corresponding object files will reside inside the module's `obj` directory, so a source file can be compiled multiple times in different modules, possibly with different compilation flags, and different objects will be created for them on each module.
...
### Build Flavors
Flavors distinguish between different build options and their corresponding output and artifacts. Flavors are defined by the `MODULE_FLAV` setting. By itself flavor only determine the module output directory. For a `MODULE_FLAV := myflav` setting, SBS will use `obj/myflav`, relative to the module directory, as the output directory. It is up to the user to put meaning into the flavor using different build options, and distinguishing it from other flavors.

The idea of flavors (in other build system other terms, like 'build configuration' are used) isn't unique to SBS and it is beyond the scope of this manual to explain how to manage you flavors, but a possible solution for can be based on different project config files, one for each flavor which all modules Makefiles include and alternating them according to the required flavor at the time (using renames, soft links, etc).

All SBS modules must have a flavor. If no flavor is provides, SBS defaults to the `dbg` flavor (see below).  Flavor names can't contain white spaces.

#### SBS Internal Flavors
SBS provides two internal flavors for debug and release which slightly modify the compilation flags with popular relevant settings:
- Debug (`dbg`) -  `-g -O0 --DDEBUG=1 -D__DEBUG__=1`
- Release (`rel`) -  `-O3`

If you are unfamiliar with these build options, consult the compilers documentation.
...
### Header dependencies
By default, SBS adds compilation flags for header files dependencies generation (`.d`, files) for better build behavior on headers modification. This behavior can be turned off by setting `MODULE_DEP_FLAGS` value to `0`. For more details about headers dependencies generation, consult the compiler's documentation.
...
### Compilers
SBS supports both GCC and Clang as compilers suite. By default, SBS uses GCC as its compiler suite. To change this behavior, set the `MODULE_CSUITE` setting value to `clang`.

## Sub modules
Each module may define sub-modules to build. A sub module is any directory that has valid Makefile in it, so it doesn't have to be a SBS module by itself. A sub module can be built before, during or after the parent module is built. Under the hood, sub modules are built by invoking `make -C`. SBS provides settings to define which and when sub modules are built. Sub-modules settings allow creating of complex project hierarchies, with complete control over build order. For example:
...
Occasionally, one might want to have explicit Makefile names in a given directory, where build are invoked using `make -f <FILENAME>`. SBS offers some support for this type of build command. A SBS Makefile can be named whatever and executed with a `make -f` command, but there a few notes to consider:
1. Submodules are always built using a default Makefile.
2. While true for Makefile in general, it's still worth mentioning that more than one Makefile in the same directory (i.e. `Makefile.A` and `Makefile.B`) are hard to maintain and should be carefully written so they won't overwrite one another output. It is best avoided.

## SBS variables and targets naming
SBS defines internally variables and make targets prefixed with `sbs` and `SBS`. When writing SBS makefiles, the author should avoid any variables and targets with these prefixes, as they might get overridden.

SBS defines many variables upon the inclusion of `moddule.inc.mk`, all can be used *after* the include directive. For example, `SBS_OBJS` contains all the object files a give SBS module creates. There are many such variables, and in some cases these might be useful to the author. To see all the internal variables, use `make sbs_dump_internals` over a your module. See the [debugging](#sbs debugging) section.

## SBS debugging
In the rare and unheard of case of a buggy build, SBS provides a couple of debugging mechanics:
### Variables dump
SBS auto defines a few targets for variables inspection so the user can use to get insights into SBS mechanics:
1. `sbs_dump_internals` - Shows all the SBS internal variables generated after the `module.inc.mk` inclusion, based on the module's Makefile
2. `sbs_dump_module_vars` - Shows the received module variables.
3. `sbs_dump_module_submodules` - Shows the module's sub-modules variables.
4. `sbs_dump_module_build_steps` - Shows all the module's build steps (i.e. Pre and post build) variables.

- All the variables shown by the `sbs_dump_internals` are available *after* the inclusion of the `module.inc.mk` file.
- The `sbs_dump_module_<TYPE>` variants are helpful when working projects where modules makefiles are generated or include other makefiles (as often happens when managing flavors). In the simple case of a self contained module, the target will just echo the module's Makefile variables.
...
yields a `-Isome_dir/include -Iother_dir/include` in the module's objects compile command.
...
yields a `-D_GNU_SOURCE -DUSE_LOG=0` in the module's objects compile command.
...
yields a `-Wno-import -Wformat` in the module's object compile command. See the compiler's documentation for possible warning settings.
...
yields a `-L/opt/external_project0 -L/opt/external_project1` flags in the link command.
...
yields a `-lm -luuid` flags in the link command.

### How to create a static or dynamic library?
A module artifact type is defined by its `MODULE_BIN_TYPE` setting. Setting it to `static` or `shared` will create static or dynamic libraries respectively. The module's artifact name will be `lib<MODULE_NAME>.a` for static libraries and `lib<MODULE_NAME>.so` for dynamic libraries.
