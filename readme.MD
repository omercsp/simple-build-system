...
SBS provides two internal flavors for debug and release which slightly modify the build flags with popular settings:
- Debug (`dbg`)
-- Adds `-g` to the compilation flags
-- Adds `-O0` to the compilation flags
-- Adds `-DDEBUG=1` and `-D__DEBUG__=1` to the compilation flags
- Release
-- Adds `-O3` to the compilation flags
